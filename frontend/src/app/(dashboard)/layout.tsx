...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const refreshNotifications = async () => {
    try {
      const [notifsData, unread] = await Promise.all([
        fetchNotifications(1, 6),
        fetchUnreadCount(),
      ]);
      setNotifications(notifsData.items);
      setTotalNotifications(notifsData.total);
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      window.addEventListener('schoolos_notification_updated', handleUpdate);
      window.addEventListener('focus', handleUpdate);
      return () => {
        clearInterval(interval);
        window.removeEventListener('schoolos_notification_updated', handleUpdate);
    ...
    async function fetchProfile() {
      try {
        ...
        if (res.ok) {
          ...
          if (json?.data) {
            if (json.data.name && json.data.name.trim() !== '') {
                ...
                <>
                  ...
                  <div className={styles.notifDropdown}>
                    <div className={styles.notifHeader}>
                      ...
                      {unreadCount > 0 && (
                        <button className={styles.notifMarkRead} onClick={handleMarkAllRead}>
                          ✓ Tandai Dibaca
                        </button>
                      )}
                    </div>
                      ...
                      {notifications.length === 0 ? (
                        <div style={{ padding: '2rem 1.25rem', textAlign: 'center', color: 'var(--text-muted)' }}>
                          ...
                          <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginTop: '0.2rem' }}>
                            Semua aktivitas akademik &amp; sistem akan muncul secara otomatis di sini.
                          </div>
                        </div>
                      ) : (
                        notifications.map((n) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className={styles.notifContent}>
                                ...
                                {n.body && (
                                  <span style={{ fontSize: '0.72rem', color: 'var(--text-secondary)', lineHeight: 1.3, marginTop: '2px' }}>
                                    {n.body}
                                  </span>
                                )}
                    ...
                    <div className={styles.notifFooter}>
                      ...
                      >
                        Lihat Seluruh Notifikasi {totalNotifications > 0 ? `(${totalNotifications})` : ''} →
                      </Link>
                    </div>
                  </div>
                </>
